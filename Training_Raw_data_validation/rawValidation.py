...
class Raw_Data_validation:
    ...
    def valuesFromSchema(self):
        ...
        """
        try:
            if not self.dbObj.isCollectionPresent('insurancefraudDB', 'train_schema'):
                with open(self.schema_path, 'r') as f:
                    dic = json.load(f)
                    f.close()
                self.dbObj.insertOneRecord('insurancefraudDB', 'train_schema', dic)
            dic = self.dbObj.getRecords('insurancefraudDB', 'train_schema')
                ...
                if (re.match(regex, filename)):
                    ...
                    if len(splitAtDot[1]) == LengthOfDateStampInFile:
                        if len(splitAtDot[2]) == LengthOfTimeStampInFile:
                            self.awsObj.copyFileToFolder(self.Batch_Directory, 'Training_Good_Raw_Files_Validated',
                                                         filename)
